...
IGraphicsLibrary::MouseEvent SDL2Graphics::getMouse()
{
    ...
    while (SDL_PollEvent(&sdlEvent)) {
        if (sdlEvent.type == SDL_MOUSEBUTTONDOWN) {
            event.x = sdlEvent.button.x / 10;
            event.y = sdlEvent.button.y / 20;
            event.pressed = true;
            event.hasEvent = true;
        ...
        if (sdlEvent.type == SDL_KEYDOWN || sdlEvent.type == SDL_KEYUP || 
            sdlEvent.type == SDL_TEXTINPUT || sdlEvent.type == SDL_QUIT) {
    ...
    int selectedGraphicIndex)
{
    ...
    drawBoxWithTitle(startX + 2 * (boxWidth + spacing), startY, boxWidth, boxHeight, "Player Name", playerBoxColor);
    ...
    drawText(startX + 2 * (boxWidth + spacing) + 2, startY + 3, displayName, playerBoxColor);
